...
You will notice that the values are separated by commas as this is a "comma separated value" (.csv) file. Additionally, this is the same data as the "beach_df_2" __data frame__ you created in last week's exercises. 
...
We know how to __read__ in a csv file with `read.csv`, now let us __read__ in a tab separated file with `read.csv`. As before first look at the file contents before __reading__ them in. It contains the sales figures of fictional clothing stores through the seasons.
